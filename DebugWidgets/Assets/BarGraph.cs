...
if (Min < 0)
            {
                //var barTransform = BarTransform.GetComponent<Transform>();
...
SetWidthPercent(value, color);
        ...
        // curr gameObject w(what its curr attached to : gameObject
        // the class of the object itself : GameObject
    }
    /// <summary>
...
half so we can
    ...
    // inital BarTransform.localScale.x is 0, so ... dont multiply
BarTransform.localScale = new Vector3(value, BarTransform.localScale.y,
BarTransform.localScale.z);
        if (signedDisplay)
        {
            BarTransform.localScale = new Vector3((value / 2) / Max,
            BarTransform.localScale.y, BarTransform.localScale.z);
        }
    }
